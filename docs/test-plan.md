...
- Startup validation refuses production external/disabled tunnel isolation without `HUB_EXTERNAL_ISOLATION_POLICY_VERIFIED=true`, rejects an invalid control-plane port, requires a strong `WG_AGENT_TOKEN` when `WG_AGENT_URL` is configured, and permits enabled kernel forwarding only while the managed or verified-external default-drop policy remains active.
