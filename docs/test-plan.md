...
- Firewall backup tests use the actual OpenSSL command to prove default encryption/decryption, key separation and root-only permissions, MAC tamper rejection, wrong-key rejection, recovery-key export/import, and absence of XML plaintext from encrypted upload requests. Explicit plaintext-mode tests prove the plugin sends readable XML only when the Hub requests `opnsense-config-plaintext-v1`.
- Device-token tests prove enrollment sets issue/expiry timestamps, expired tokens are rejected, heartbeat indicates when rotation is due, the authenticated rotation endpoint returns a replacement token and stores only its hash, and the OPNsense plugin saves rotated tokens before later authenticated uploads.
- Scheduler-lock tests prove health, firmware, and retention loops are wrapped by stable PostgreSQL advisory-lock keys, release acquired locks, avoid releasing locks they did not acquire, bypass locking only for non-PostgreSQL development databases, and reject invalid lock polling intervals.
- Plugin heartbeat tests prove explicit primary-heartbeat revocation (`410 Gone`) removes local tunnel/state, while ordinary heartbeat `401`, firmware-report `404`, and backup-upload `401` failures do not remove local artifacts.
- Plugin configuration tests prove unavoidable direct `config.xml` updates lock before parsing, keep the lock through atomic same-directory replacement, create a timestamped backup, and remove temporary files after failed writes without replacing the live configuration.
- Hub tests prove capability gating, pending-request enforcement, strict encrypted-envelope validation, explicit plaintext-envelope validation when `CONFIG_BACKUP_MODE=plaintext`, server-time retention, default plaintext upload rejection, no-store download for both `.opnenc` and `.xml`, Hub export/restore without a legacy `content` field, restore rejection for missing tables, manifest row-count mismatches, invalid required fields/foreign keys, staging database failures before existing rows are deleted, and post-commit WireGuard reconciliation failures after database/files are already consistently restored.
...
7. Run the heartbeat configd action and confirm plugin `0.2` advertises `opnsense-config-encrypted-v1` and `opnsense-config-plaintext-v1`; confirm an older/non-advertising heartbeat receives `backup_requested=false` for the Hub-selected format.
8. In default encrypted mode, export the backup recovery key to offline storage, compare its key fingerprint with `key-info`, import it into a disposable replacement environment, and decrypt a downloaded `.opnenc` backup. Verify tampering and a wrong key fail before plaintext is written. If `CONFIG_BACKUP_MODE=plaintext` is explicitly enabled, verify backup downloads are readable `.xml` and that Hub storage/export retention treats them as sensitive firewall secrets.
