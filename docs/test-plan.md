...
- Hub tests prove capability gating, pending-request enforcement, strict encrypted-envelope validation, server-time retention, plaintext upload rejection, opaque no-store download, Hub export/restore without a `content` field, and restore rejection for missing tables, manifest row-count mismatches, invalid required fields/foreign keys, and staging database failures before existing rows are deleted.
