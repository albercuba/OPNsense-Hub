...
- WireGuard peers are managed by a small validated wrapper around the agent API and sidecar-local `wg set` calls. WebGUI health probes are also delegated to the sidecar when `WG_AGENT_URL` is configured so the unprivileged web container never needs tunnel namespace access.
...
- Opening a firewall starts with a CSRF-protected `POST /devices/{device_id}/proxy/open`. With the default `FIREWALL_ACCESS_MODE=connector`, after dashboard session and company-scoped RBAC checks, the Hub creates a random, short-lived, device-scoped connector token bound to the issuing dashboard session, stores only its hash, writes a `device.connector.open` audit event, and returns `no-store` instructions.
...
- The Hub accepts binary WSS frames only and copies bytes between WSS and the firewall TCP socket. In connector mode, it does not terminate the browser-to-firewall TLS session, parse HTTP, inject cookies, or receive OPNsense credentials.
- With explicit `FIREWALL_ACCESS_MODE=hub_proxy`, the same Open action writes a `device.hub_proxy.open` audit event and redirects the browser to authenticated `/proxy/devices/{device_id}/` routes on `PUBLIC_URL`. Those routes enforce dashboard session and company RBAC, validate that the target is the device's WireGuard `/32`, isolate OPNsense cookies with a device-specific Hub cookie prefix, rewrite same-firewall redirects back under `/proxy/devices/{device_id}`, and delegate HTTP(S) requests to the WireGuard sidecar when `WG_AGENT_URL` is configured. This avoids installing software on administrator computers, but it changes the security boundary: the Hub web process and sidecar proxy WebGUI HTTP(S) and can see request paths, headers, cookies, bodies, and response content. `PROXY_VERIFY_TLS=true` preserves upstream TLS validation from the sidecar to OPNsense and should remain enabled unless the operator explicitly accepts weaker upstream TLS verification.
- In connector mode, `/proxy/bootstrap` and `/proxy/devices/*` are not routed. `PROXY_PUBLIC_URL` now supplies only the optional raw relay's base DNS hostname; it is not an L7 browser proxy origin.
...
- In default connector mode, the Hub never receives OPNsense administrator passwords, WebGUI session cookies, or plaintext WebGUI requests because firewall TLS terminates only at OPNsense.
