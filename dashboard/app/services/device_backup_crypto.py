...
def _invalid_encrypted_backup(detail: str) -> HTTPException:
...
def validate_encrypted_device_backup(
    ...
    if not isinstance(value, dict) or set(value) != _ENVELOPE_FIELDS:
        raise _invalid_encrypted_backup("encrypted backup envelope fields are invalid")
    if value.get("format") != ENCRYPTED_DEVICE_BACKUP_FORMAT:
        raise _invalid_encrypted_backup("encrypted backup format is not supported")
    version = value.get("version")
    iterations = value.get("iterations")
    if not isinstance(version, int) or isinstance(version, bool):
        raise _invalid_encrypted_backup("encrypted backup version is invalid")
    if not isinstance(iterations, int) or isinstance(iterations, bool):
        raise _invalid_encrypted_backup("encrypted backup KDF parameters are invalid")
    if (
        ...
        raise _invalid_encrypted_backup("encrypted backup algorithms are not supported")

    try:
        source_device_id = uuid.UUID(str(value.get("device_id")))
    except (ValueError, AttributeError, TypeError) as exc:
        raise _invalid_encrypted_backup("encrypted backup device ID is invalid") from exc
    if source_device_id != expected_device_id:
        raise _invalid_encrypted_backup("encrypted backup is for another firewall")

    key_id = value.get("key_id")
    mac = value.get("mac")
    if not isinstance(key_id, str) or not _KEY_ID_RE.fullmatch(key_id):
        raise _invalid_encrypted_backup("encrypted backup key fingerprint is invalid")
    if not isinstance(mac, str) or not _HEX_64_RE.fullmatch(mac):
        raise _invalid_encrypted_backup("encrypted backup authentication code is invalid")
    ...
    if (
        ...
        raise _invalid_encrypted_backup("encrypted backup hostname is invalid")
    captured_at = value.get("captured_at")
    if not isinstance(captured_at, str) or len(captured_at) > 64:
        raise _invalid_encrypted_backup("encrypted backup timestamp is invalid")
    try:
        parsed_timestamp = datetime.fromisoformat(captured_at.replace("Z", "+00:00"))
    except ValueError as exc:
        raise _invalid_encrypted_backup("encrypted backup timestamp is invalid") from exc
    if parsed_timestamp.tzinfo is None:
        raise _invalid_encrypted_backup("encrypted backup timestamp must include a timezone")

    encoded_ciphertext = value.get("ciphertext")
    if not isinstance(encoded_ciphertext, str) or not encoded_ciphertext:
        raise _invalid_encrypted_backup("encrypted backup ciphertext is invalid")
    if len(encoded_ciphertext) > ENCRYPTED_DEVICE_BACKUP_MAX_ENCODED_CIPHERTEXT_CHARS:
        raise _invalid_encrypted_backup("encrypted backup ciphertext is too large")
    try:
        ciphertext = base64.b64decode(encoded_ciphertext, validate=True)
    except (ValueError, binascii.Error) as exc:
        raise _invalid_encrypted_backup("encrypted backup ciphertext is invalid") from exc
    if not ciphertext.startswith(b"Salted__"):
        raise _invalid_encrypted_backup("encrypted backup salt header is invalid")
    encrypted_body = ciphertext[16:]
    if not encrypted_body or len(encrypted_body) % 16 != 0:
        raise _invalid_encrypted_backup("encrypted backup ciphertext length is invalid")
    ...
    if len(canonical_payload) > ENCRYPTED_DEVICE_BACKUP_MAX_ENVELOPE_CHARS:
        raise _invalid_encrypted_backup("encrypted backup envelope is too large")
...
def encrypted_device_backup_filename(hostname: str, received_at: datetime) -> str:
    ...
    return (
        f"{safe_hostname or 'firewall'}-backup-"
        f"{received_at.strftime('%Y%m%d%H%M%S')}.opnenc"
    )
